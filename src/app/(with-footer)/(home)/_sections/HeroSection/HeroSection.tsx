...
export const HeroSection = () => {
	...
	const sectionRef = useRef(null); // Keep the section ref for scoping

	useEffect(() => {
		if (!sectionRef.current) return; // Ensure sectionRef is available

		const tl = gsap.timeline();

		const elements = {
			section: sectionRef.current, // The section itself
			title: sectionRef.current.querySelector(`.${styles.title}`), // Select using CSS module class
			subtitle: sectionRef.current.querySelector(`.${styles.subtitle}`),
			arrow: sectionRef.current.querySelector(`.${styles.arrow}`),
			language: sectionRef.current.querySelector(`.${styles.language}`),
			introSubtitles: sectionRef.current.querySelectorAll(`.${styles.introSubtitle}`), // Select multiple with querySelectorAll
			buttonContainer: sectionRef.current.querySelector(`.${styles.buttonContainer}`),
			actions: sectionRef.current.querySelector(`.${styles.actions}`),
			copyEmailButton: sectionRef.current.querySelector(`.${styles.copyEmailButton}`),
			chatbot: sectionRef.current.querySelector('chatbot-container'), // Assuming your Chatbot component has this class
			arrowwave: sectionRef.current.querySelector(`.${styles.arrowwave}`),
			scrollToTopButton: sectionRef.current.querySelector('scrollToTopButton-container'), // Assuming your ScrollToTopButton has this class
		};


		tl.fromTo(elements.section, { opacity: 0 }, { opacity: 1, duration: 0.8, ease: "power2.out" })
			.fromTo(elements.title, { y: 50, opacity: 0 }, { y: 0, opacity: 1, duration: 1, ease: "power3.out" }, "-=0.6")
			.fromTo(elements.subtitle, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, ease: "power2.out" }, "-=0.4")
			.fromTo(elements.arrow, { opacity: 0, scale: 0.8 }, { opacity: 1, scale: 1, duration: 0.7, ease: "back.out(1.7)" }, "-=0.3")
			.fromTo(elements.language, { opacity: 0, x: -20 }, { opacity: 1, x: 0, duration: 0.6, ease: "power2.out" }, "-=0.5")
			.fromTo(elements.introSubtitles, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.7, ease: "power2.out", stagger: 0.2 }, "-=0.4")
			.fromTo(elements.buttonContainer, { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 0.6, ease: "back.out(1.7)" }, "-=0.3")
			.fromTo(elements.actions, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6, ease: "power2.out" }, "-=0.4")
			.fromTo(elements.copyEmailButton, { opacity: 0, x: -20 }, { opacity: 1, x: 0, duration: 0.5, ease: "power2.out" }, "-=0.3")
			.fromTo(elements.chatbot, { opacity: 0, x: 20 }, { opacity: 1, x: 0, duration: 0.5, ease: "power2.out" }, "-=0.3") // Animate Chatbot
			.fromTo(elements.arrowwave, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.7, ease: "power2.out" }, "-=0.4")
			.fromTo(elements.scrollToTopButton, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, ease: "power2.out" }, "-=0.3"); // Animate ScrollToTopButton
	...
	return (
		<section aria-labelledby="hero-title" className={styles.section} ref={sectionRef}>
			<h1 id="hero-title" className={`text-gradient ${styles.title}`}> {/* Class here */}
				Max Comperatore
			</h1>
			<h2 className={styles.subtitle}> {/* Class here */}
				{positions[positionIndex]}
			</h2>

			<svg className={styles.arrow} width="144" height="141" viewBox="0 0 144 141" fill="none" xmlns="http://www.w3.org/2000/svg"> {/* Class here */}
			...
			<p className={styles.language}> {/* Class here */}
				{t("language")}
			</p>

			<div className={styles.intro}> {/* Class here */}
				<p className={styles.introSubtitle}> {/* Class here */}
					{t.rich("subtitle", {
			...
			<div className={styles.buttonContainer}> {/* Class here */}
			...
			<div className={styles.actions}> {/* Class here */}
				<NextLink
					href="https://drive.google.com/file/d/1qDZWM229XnPH3q5i1TSbivZB5OGbcacs/view?usp=sharing"
					target="_blank"
					prefetch={true}
					passHref
				>
